...
interface SocietyMemberKYCFormProps {
...
const SocietyMemberKYCForm: React.FC<SocietyMemberKYCFormProps> = ({
  onSubmit,
  onCancel,
  isLoading,
  error,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!panCardImage) {
      errors.panCardImage = 'PAN card image is required';
    }
    ...
    if (panCardImage) {
      submitData.append('panCardImage', panCardImage);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          {validationErrors.panNumber && (
            <p className="mt-2 text-sm text-red-600">{validationErrors.panNumber}</p>
          )}
        </div>
        ...
        <FileUpload
          onFileSelect={setPanCardImage}
          accept="image/*"
          maxSize={5}
          label="PAN Card Image"
          required
          error={validationErrors.panCardImage}
        />
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <ul className="text-sm text-blue-800 space-y-1">
            <li>• Ensure all document images are clear and readable</li>
            <li>• File size should not exceed 5MB per document</li>
            <li>• Accepted formats: JPG, PNG, PDF</li>
            <li>• Verification may take 1-2 business days</li>
            <li>• Both Aadhar and PAN documents are required for society members</li>
          </ul>
        </div>
...
export default SocietyMemberKYCForm;
